...
unsigned long flash_init(void) {
	...
	/* Setup offsets */
	flash_get_offsets((vu_long *) CFG_FLASH_BASE, &flash_info[0]);
...
static void flash_get_offsets(ulong base, flash_info_t *info) {
	...
	if (((info->flash_id & FLASH_VENDMASK) == FLASH_MAN_SST)
		...
		while (i < info->sector_count) { /* 64k regular sectors	*/
			...
			|| ((info->flash_id & FLASH_TYPEMASK) == FLASH_ATBV642DT)) {
...
void flash_print_info(flash_info_t *info) {
	...
	case FLASH_ATBV642DT:
		printf("AT49BV642DT (64 Mbit, top boot sector)\n");
		break;
	case FLASH_AM400B:
...
static ulong flash_get_size(vu_long *addr, flash_info_t *info) {
	...
	switch (value) {
	case (CFG_FLASH_WORD_SIZE) ATM_MANUFACT:
	...
	case (CFG_FLASH_WORD_SIZE) ATM_ID_BV642DT:
		info->flash_id += FLASH_ATBV642DT;
	...
	for (sect = s_first; sect <= s_last; sect++) {
		if (info->protect[sect] == 0) { /* not protected */
			...
			if ((info->flash_id & FLASH_VENDMASK) == FLASH_MAN_SST) {
				...
				if ((sect == s_first) || ((info->flash_id & FLASH_TYPEMASK)
						== FLASH_ATBV642DT)) {
					addr[CFG_FLASH_ADDR0] = (CFG_FLASH_WORD_SIZE) 0x00AA00AA;
					addr[CFG_FLASH_ADDR1] = (CFG_FLASH_WORD_SIZE) 0x00550055;
					addr[CFG_FLASH_ADDR0] = (CFG_FLASH_WORD_SIZE) 0x00800080;
...
static int write_word(flash_info_t *info, ulong dest, ulong data) {
	...
	for (i = 0; i < 4 / sizeof(CFG_FLASH_WORD_SIZE); i++) {
		...
		/* data polling for D7 */
		start = get_timer(0);
		while ((dest2[i] & (CFG_FLASH_WORD_SIZE) 0x00800080) != (data2[i]
				& (CFG_FLASH_WORD_SIZE) 0x00800080)) {
